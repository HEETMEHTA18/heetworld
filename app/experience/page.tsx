...
export default function ExperiencePage() {
  const active = experience.filter((item) => !item.legacy);
  const archived = experience.filter((item) => item.legacy);
          ...
          <div className="mt-20">
            <div className="mb-8 flex items-center gap-3">
              <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
                Archived ({archived.length}) — earlier work, kept for the record
              </span>
              <div className="h-px flex-1 bg-border" />
            </div>
            <Timeline items={archived} muted />
          </div>
        )}
      </Container>
    </>
...
function Timeline({ items, muted = false }: { items: ExperienceItem[]; muted?: boolean }) {
  return (
